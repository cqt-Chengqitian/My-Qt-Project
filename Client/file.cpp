...
{
    qDeleteAll(m_pFileInfoList);
    m_pFileInfoList.clear();// 当改变路径的时候把之前的目录的内容给删除

    m_pFileInfoList = pFileList;
    ui->listWidget->clear(); // 刷新之前清空
    foreach(FileInfo* pFileInfo, pFileList){ // 将目录下的文件显示到客户端的文件展示列表
...
void File::uploadFile()
{
    ...
    // 上传完成（finshed）后，销毁uploader自身
    connect(uploader,&Uploader::finshed,uploader,&Uploader::deleteLater);
    // 线程结束后也触发uploader销毁
    connect(uploader->thread(),&QThread::finished,uploader,&Uploader::deleteLater);
