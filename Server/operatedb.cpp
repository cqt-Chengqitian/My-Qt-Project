...
bool OperateDB::handleLogin(const char *name, const char *pwd)
{
    ...
    QString sql = QString("select * from user_info where name = '%1' and pwd = '%2'")
            .arg(name).arg(pwd); // 查询用户与密码是否正确
    qDebug()<<"检查用户是否存在"<<endl<<sql;
    QSqlQuery q; // 通过该对象执行sql语句
...
bool OperateDB::agreeHandleaddfriend(const char *curName, const char *tarName)
{
    ...
    QString sql = QString("insert into friend(user_id,friend_id)"
                          "select ul.id, u2.id from user_info ul, "
                          "user_info u2 where ul.name='%1' and u2.name='%2'").arg(curName).arg(tarName);
    qDebug()<<"添加好友记录:"<<endl<<sql;
    QSqlQuery q;
    return q.exec(sql);
}
...
QStringList OperateDB::handleFlushfriend(const char *curName)
{
    ...
    QString sql = QString("select name from user_info where id in "
                          "(select friend_id from friend where user_id ="
                          "(select id from user_info where name='%1')"
                          "union select user_id from friend where friend_id ="
                          "(select id from user_info where name='%1'))").arg(curName);
    qDebug()<<"刷新好友"<<endl<<sql;
    QSqlQuery q;
    q.exec(sql);
    while(q.next()){
